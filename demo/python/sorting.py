    ...
    # 所以，“从第(n-1)个元素倒数到第2个元素”就是range(n-2, 0, -1)
        ...
        # 对于每一轮，j的值从0变化到i
            if (list[j] > list[j + 1]):
                ...
                list[j] = list[j + 1]
                list[j + 1] = temp
        print(list) # 检查每一轮排序的效果。真实排序时，可以注释掉这一句
...
def insertion_sort(list):
    for i in range(1, len(list)):
        for j in range(i):
            if (list[j] >= list[i]):
                list.insert(j, list.pop(i)) # 取出第i号元素，直接插入第j号元素的前边
                break # 一旦完成插入，直接结束 for 循环
        print(list[:i+1], list[i+1:], sep='   ')
