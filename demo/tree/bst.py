...
class BST(Generic[V]):
    ...
    def __loc(self, node: BSTNode, key: V) -> BSTNode:
        if key == node.val:
            return node
        if key < node.val:
    ...
    def __add(self, node: BSTNode, value: Any) -> BSTNode:
        if not node:
            return BSTNode(value)
        if value < node.val:
            node.pleft = self.__add(node.pleft, value)
        elif value > node.val:
            node.pright = self.__add(node.pright, value)
        else:
            node.count += 1
        return node
    ...
    def insert_nonrecursive(self, value):
    ...
    def __del(self, node: BSTNode, key: Any) -> BSTNode:
        """
        ...
        if key < node.val: # 如果待删除节点可能在左子树
            ...
            node.set_left(self.__del(node.pleft, key) if node.pleft else None)
            return node # 有改变也在左子树, 当前根节点肯定不动. 直接返回.
        if key > node.val:
            node.set_right(self.__del(node.pright, key) if node.pright else None)
            return node
    ...
    def delete_nonrecursive(self, key):
